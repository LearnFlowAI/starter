...
- 每次 `git commit` 前 `pnpm lint` 会自动运行，`git push` 前 Husky 会再跑一次 `pnpm test`（`vitest` 在配置中设置为单线程运行，避免多线程竞争）。如果遇到钩子报错，先确认 `pnpm install` 已完成并使用与 `pnpm-lock.yaml` 兼容的 Node/Pnpm 版本。
