...
- [x] 实现基础积分计算（calculateSessionPoints）
  ...
  - ✅ 暂停惩罚：每次暂停 -2 分
  - ✅ 最低保底：1 分
- [x] 扩展积分规则（参考 RecordEntry 字段）
  - [x] 质量评分加成（rating 1-5）
  - [x] 错题惩罚（mistakeCount）
  - [x] 书写质量加成（writingStars 0-3）
  - [x] 学习习惯奖励（复习/订正/预习）
...
### 交互状态
- [x] 空状态设计（无任务、无记录、无数据时的引导）
- [ ] 加载态设计（数据加载中的骨架屏/spinner）
- [ ] 错误态设计（网络错误、数据异常的提示）
- [x] 成功/失败反馈（toast/snackbar）
...
- [x] lib/export.ts 测试（导出格式与字段）
- [x] lib/migration.ts 测试（中断迁移与兼容）
- [ ] 扩展测试覆盖率
  - [ ] models.ts 类型约束测试
  - [ ] defaults.ts 默认值测试
